...
select * from loc;
select a.assembly1 from loc a inner join(
select loc_from,loc_to,count(loc_to) from trips
group by loc_from , loc_to
order by count(loc_to) desc limit 2)b
on a.id = b.loc_to;
...
select sum(fare),driverid from trips
group by driverid
...
group by duration
order by count(tripid)  desc limit 1;
...
select sum(searches_got_estimate)*1.0/sum(searches) from trips_details4;

-- estimate to search for quote rates
select sum(searches_for_quotes)*1.0/sum(searches) from trips_details4;


-- quote acceptance rate

select sum(searches_got_quotes)*1.0/sum(searches) from trips_details4;

-- quote to booking rate
select sum(searches_got_quotes)*1.0/sum(driver_not_cancelled+customer_not_cancelled) from trips_details4;

-- booking cancellation rate
select count(*)-sum(driver_not_cancelled+customer_not_cancelled)/sum(searches) from trips_details4;

-- conversion rate
select sum(end_ride)/sum(searches) from trips_details4;
...
select sum(fare),loc_from from trips
group by loc_from
order by sum(fare) desc limit 1;

select loc_from,count(*) - sum(driver_not_cancelled) from trips_details4
group by loc_from
order by count(*) - sum(driver_not_cancelled) desc limit 1;

select loc_from,count(*) - sum(customer_not_cancelled) from trips_details4
group by loc_from
order by count(*) - sum(customer_not_cancelled) desc limit 1;


-- which duration got the highest trips and fares

select duration,sum(fare) from trips
group by duration
order by sum(fare) desc limit 1;

select duration,count(distinct tripid) from trips
group by duration
order by count(distinct tripid) desc limit 1;
